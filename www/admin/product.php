<?php
	...
	if ($_SESSION['auth_admin'] == "yes_auth") 
	{
		...
		$_SESSION['urlpage'] = "<a href= 'index.php'>√лавна€</a> \ <a href= 'product.php'>“овары</a>"; // в сессию помещаем ссылку дл€ навигационной цепочки

		include("include/db_connect.php"); // подключаемс€ к бд
...
<div class="block-body">
	...
	<div class="block-content">
        ...
        <p class="cout-style">¬сего товаров: <strong><?php echo $count_products;?></strong></p>
        </div>
                        ...
                        do
                        {
                            ...
                            <li class = "product-li">
                                ...
                                <p align="center" class="link-action">
                                    <a class="green" href="edit_product.php?id='.$row["products_id"].'">»зменить</a> | <a rel="product.php?'.$url.'id='.$row["products_id"].'&action=delete" class="delete" > ”далить</a>
                                </p>
                            </li>                            
                            ';
...
if ($page != 1) $pervpage = '<li><a class="pstr-prev" href="product.php?'.$url.'page='. ($page - 1) .'" />Ќазад</a></li>';
 
if ($page != $total) $nextpage = '<li><a class="pstr-next" href="product.php?'.$url.'page='. ($page + 1) .'"/>¬перЄд</a></li>';
 
// Ќаходим две ближайшие станицы с обоих краев, если они есть
...
}else
{
    $strtotal = ""; 
            ...
            ?>
            <div class="footerfix"></div> <!-- чтобы навигаци€ не обтекалась с товарами -->
            <?php
    if ($total > 1)
{
    echo '
